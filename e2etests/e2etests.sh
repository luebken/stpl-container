...
printf "Similarity: "
curl -s -X GET -d @e2etests/example-2-vertx-openshift-missing-dependency-effective-pom.xml localhost:8088/recommendation | jq .Similarity
curl -s -X GET -d @e2etests/example-2-vertx-openshift-missing-dependency-effective-pom.xml localhost:8088/recommendation | jq .RecommendationItems
